...
if(isset($empresa) && isset($pregunta)){
	...
	$pregunta = $mysql->Select("select * from preguntas where ID_preguntas='$pregunta'");
}
?>
...
if(isset($accion) && $accion=="guardar-nota"){
	$notas = $mysql->Select("select ID_notas,nota,comentario from notas where ID_items='".$id_item."' and ID_clientes='".$id_cliente."'");

	if($notas[0][0]=="")	
		$mysql->Insert("insert into notas values('0','$id_item','$id_cliente','$nota','$comentario')");
	else
		$mysql->Insert("update notas set nota='$nota', comentario='$comentario' where ID_notas='".$notas[0][0]."'");
...
if(isset($accion) && $accion=="agregar-mejoras"){
	...
	$mysql->Insert("insert into mejoras values('0','$id_item','$id_cliente','$mejora','$complejidad','$beneficio')");
...
if(isset($borrarmejora)){
	$mysql->Delete("delete from mejoras where ID_mejoras='$borrarmejora'");
}
?>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#E8E8E8">
  ...
  <? $items = $mysql->Select("select * from items where ID_preguntas='".$pregunta[0][1]."' order by orden_item asc");
 	if(count($items)!=0){
 ...
 ?>
  ...
  <tr>
	...
	?>
     ...
     <form action="pregunta.php?idcliente=<?=$id_empresa?>&idpregunta=<?=$id_pregunta?>&item=<?=$Valor[0]?>" method="post">
      ...
      <tr>
        ...
        <? $nota = $mysql->Select("select nota,comentario from notas where ID_items='".$Valor[0]."' and ID_clientes='".$empresa[0][0]."'")?>
      ...
      <tr>
        ...
        <? $mejoras = $mysql->Select("select * from mejoras where ID_items = '$Valor[0]' and ID_clientes = '".$empresa[0][0]."' ");
 	if(count($mejoras)!=0){
 ...
 ?> 
          <tr>
            ...
            <td align="center"><a href="pregunta.php?idcliente=<?=$id_empresa?>&idpregunta=<?=$id_pregunta?>&borrarmejora=<?=$Valor2[0]?>&item=<?=$Valor[0]?>"><img src="images/delete.gif" width="16" height="16" border="0" /></a></td>
          </tr>
  ...
  <tr>
    ...
    <?
		...
		*/
		...
		$p = $mysql->Select("select num from preguntas where ID_preguntas='$id_pregunta'");
		...
		while($condicion){
			...
			$numero_pregunta -= 1;
			//echo " - $numero_pregunta";
			$p = $mysql->Select("select ID_preguntas from preguntas where num='$numero_pregunta'");
   ...
   <h2> <a href="pregunta.php?idcliente=<?=$id_empresa?>&idpregunta=<?=$id_pregunta_nuevo?>">Pregunta Anterior</a></h2>
    <? } ?>
    
    </td>
    <td width="33%" align="center"><h2><a href="evaluar2.php?id=<?=$empresa[0][0]?>">Lista de Preguntas</a></h2></td>
    ...
    <? if($id_pregunta_nuevo!=""){ ?>
   <h2> <a href="pregunta.php?idcliente=<?=$id_empresa?>&idpregunta=<?=$id_pregunta_nuevo?>">Pregunta Siguiente</a></h2>
    <? } ?>
    </td>
  </tr>
</table>
