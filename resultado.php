...
<h1>Resultados para  
  <select name="empresas" class="select-empresa" id="empresas" onchange="MM_jumpMenu('parent',this,0)">
  <option value="resultado.php">Seleccione Empresa a Evaluar</option>
 ...
 ?>
  ...
  <option value="resultado.php?id=<?=$Valor[0]?>" <? if($_GET['id']==$Valor[0]){ ?> selected="selected" <? } ?>><?=$Valor[3]?></option>
...
<br />
 ...
 ?>
  ...
  <tr>
    ...
    <div style="display:none"><div id="pregunta_extendida_<?=$Valor[0]?>" <? if($estado=="0"){ echo "style=\"display:none\""; } ?>>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="151" rowspan="6" align="center" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td align="center"><h3>Estado de Avance:</h3>
			  ...
			  $Items = $mysql->Select("select COUNT(*) from items where ID_preguntas='".$Valor[0]."'");
			  ...
			  $Respuestas = $mysql->Select("
										   	SELECT COUNT(*) 
											FROM notas, items 
											WHERE notas.ID_clientes = '$id_empresa' 
											and items.ID_Preguntas = '$Valor[0]'
          ...
          <tr>
            <td align="center"><h2><a href="pregunta.php?idcliente=<?=$empresa[0][0]?>&idpregunta=<?=$Valor[0]?>">Responder</a></h2></td>
          </tr>
        </table></td>
        </tr>
      <tr>
        <td align="left">Elemento:</td>
        <td align="left"><?=$Valor[3]?></td>
        </tr>
      <tr>
        <td align="left">Sub Elemento:</td>
        <td align="left"><?= ($Valor[4])?></td>
        </tr>
      <tr>
        <td align="left">Practica:</td>
        <td align="left"><?= ($Valor[5])?></td>
        </tr>
      <tr>
        <td align="left">Pregunta:</td>
        <td align="left"><?=($Valor[6])?></td>
        </tr>
      <tr>
        <td align="left">Intencion de la Pregunta:</td>
        <td align="left"><?= $Valor[7]?></td>
        </tr>
    </table>
    </div></div>
    </td>
