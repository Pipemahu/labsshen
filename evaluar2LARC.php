...
<h1>Evaluacion para  <select name="empresas" class="select-empresa" id="empresas" onchange="MM_jumpMenu('parent',this,0)">
  <option value="evaluar2LARC.php">Seleccione Empresa a Evaluar</option>
 ...
 ?>
  ...
  <option value="evaluar2LARC.php?id=<?=$Valor[0]?>" <? if($_GET['id']==$Valor[0]){ ?> selected="selected" <? } ?>><?=utf8_encode($Valor[3])?></option>
        ...
        <td width="151" rowspan="6" align="center" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td align="center"><h3>Estado de Avance:</h3>
			  ...
			  $Items = $mysql->Select("select COUNT(*) from items_LARC where ID_preguntas='".$Valor[0]."'");
			  ...
			  $Respuestas = $mysql->Select("
										   	SELECT COUNT(*) 
											FROM notas_LARC, items_LARC
											WHERE notas_LARC.ID_clientes = '$id_empresa'
											and items_LARC.ID_Preguntas = '$Valor[0]'
          ...
          <tr>
            <td align="center"><h2><a href="preguntaLARC.php?idcliente=<?=$empresa[0][0]?>&idpregunta=<?=$Valor[0]?>">Responder</a></h2></td>
          </tr>
        </table></td>
        </tr>
      <tr>
